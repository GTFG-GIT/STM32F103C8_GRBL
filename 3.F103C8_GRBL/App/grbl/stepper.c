...
void HandleStepSetIT(void)
{
  ...
  if (st.step_outbits & STEP_X_Pin)
  {
    actpos[0] += (st.dir_outbits & DIR_X_Pin) ? 1 : -1;
  }
  
  if (st.step_outbits & STEP_Y_Pin)
  {
    actpos[1] += (st.dir_outbits & DIR_Y_Pin) ? 1 : -1;
  }
  
  if (st.step_outbits & STEP_Z_Pin)
  {
    actpos[2] += (st.dir_outbits & DIR_Z_Pin) ? 1 : -1;
  }
